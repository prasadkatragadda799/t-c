...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Programs View More / View Less Toggle ---
    const programsToggleBtn = document.getElementById('programsToggleBtn');
    const programsGrid = document.querySelector('.programs-grid');
    if (programsToggleBtn && programsGrid) {
        programsToggleBtn.addEventListener('click', () => {
            const isExpanded = programsGrid.classList.toggle('expanded');
            if (isExpanded) {
                programsToggleBtn.innerHTML = 'View Less Programs <span class="arrow">↑</span>';
            } else {
                programsToggleBtn.innerHTML = 'View More Programs <span class="arrow">↓</span>';
                const programsSection = document.getElementById('programs');
                if (programsSection) {
                    const offset = document.querySelector('.header').offsetHeight + 20;
                    const top = programsSection.getBoundingClientRect().top + window.pageYOffset - offset;
                    window.scrollTo({ top, behavior: 'smooth' });
                }
            }
        });
